...
def IoU_CVFDLO():
    ...
    for img_name in img_list:
        pred = cv2.imread(os.path.join('../data/test_predicts_mask_R101/250430/with all', img_name), cv2.IMREAD_GRAYSCALE)
...
def dice_CVFDLO():
    ...
    dice_dict = {}
    for img_name in img_list:
        pred = cv2.imread(os.path.join('../data/test_predicts_label/test5', img_name), cv2.IMREAD_GRAYSCALE)
        target = cv2.imread(os.path.join('../data/test_labels', img_name), cv2.IMREAD_GRAYSCALE)
        target = cv2.resize(target, (640, 360))
        pred[pred < 31] = 0
        pred[pred != 0] = 1
