...
class Skeletonize():
    ...
    def run(self, mask, density=7, source_img=None, verbose=False, times=None):
        ...
        self.IMG_H, self.IMG_W = mask.shape[0], mask.shape[1]

        # mask = cv2.dilate(mask, kernel=(self.kernel_size, self.kernel_size), iterations=10)
        # mask = cv2.erode(mask, kernel=(self.kernel_size, self.kernel_size), iterations=15)
        ...
        if self.if_debug:
            ...
            self.showPoints_dict(skeleton_rf, ints = ints_dict_rf)


        # 端点信息绑定(孤立端点,交叉端点,端点隶属DLO编号),端点绑定到交叉点
        ends_dict_rf, ints_dict_rf = self.constructEndsDict(routes, routes_im, source_img, ends_rf, ints_dict_rf)
        ...
        return skeleton_rf, routes, end_pairs, ends_dict_rf, ints_dict_m, []
    ...
    def extractRoutes(self, num_labels, labels, skel_img):
        ...
        for n in range(1, num_labels):
            ...
            if len(ends_f) == 2:
                ...
                if len(route) > 0:
                    routes[n] = {'route': route.tolist(), 'ends': []}
        return routes
    ...
    def prune_short_routes(self, routes):
        del_list = []
        for i in routes.keys():
            if len(routes[i]['route']) < max(routes[i]['width'][1] * 1.3, 20) \
                    or routes[i]['width'][0] < self.total_mean_width * 0.3:
                if self.if_debug:
                    print(len(routes[i]['route']))
                    print(routes[i]['width'][0])
                    print(self.total_mean_width * 0.2)
                del_list.append(i)
    ...
    def constructEndsDict(self, routes, routes_im, source_img, ends, ints_dict):
        ...
        for i, end in enumerate(ends_list):
            ...
            ends_dict[i] = {"point": end,
                            "route_label": routes_im[end[0]][end[1]].item(),
                            "point_label": i,
                            "pair_ends": [],
                            "point_type": end_type,
    ...
    def calcRouteDirection(self, routes_im, ends_dict_rf, type='iso'):
        ...
        for i, end_dict in ends_dict_rf.items():
            ...
            end_window = routes_im[max(0, (end_point[0] - 2 * self.kernel_size)):min(IMG_H, (
                        end_point[0] + 2 * self.kernel_size)),
                         max(0, (end_point[1] - 2 * self.kernel_size)):min(IMG_W,
                                                                           (end_point[1] + 2 * self.kernel_size))]
    ...
    def calcEndSimilarity(self, end_dict_1, end_dict_2, skel, flag='end_pair'):
        ...
        if self.if_debug:
            ...
            if flag == 'int_pair':
                multi = 10
                back = back[max(0, ((point1[0] + point2[0])//2 - multi*self.kernel_size)):min(((point1[0] + point2[0])//2 + multi*self.kernel_size), self.IMG_H-1),
                            max(0, ((point1[1] + point2[1])//2 - multi*self.kernel_size)):min(((point1[1] + point2[1])//2 + multi*self.kernel_size), self.IMG_W-1)]
    ...
    def handleIntersections(self, skel, ends_dict_rf, ints_dict_m):
        ...
        for i, int_dict in ints_dict_m.items():
            ...
            if int_end_num == 1:
                ...
                end_pairs += self.handleTwo(skel, ends_dict_rf, int_dict)
                to_delete.append(i)
            elif int_end_num == 3:
    ...
    def handleTwo(self, skel, ends_dict_rf, int_dict):
        print('handleTwo')
        end_pairs = []
    ...
    def mergeIntsFromRoutes(self, routes, ints_dict):
        ...
        for dkey in diff:
            ...
            point = tuple([np.int32(np.mean(x)), np.int32(np.mean(y))])
