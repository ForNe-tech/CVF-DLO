...
class BS_curve(object):
    ...
    def __init__(self, n, p, k, cp=None, knots=None):
    ...
    def De_Boor(self, uq):
        ...
        P = self.cp[k - self.k:k - sk + 1]
        P = P.copy()
        dis = k - self.k  # the index distance between storage loaction and varibale i
        # 1-h

        for r in range(1, h + 1):
            # k-p >> k-sk
            temp = []  # uesd for Storing variables of the current stage
            for i in range(k - self.k + r, k - sk + 1):
                a_ir = (uq - self.u[i]) / (self.u[i + self.k - r + 1] - self.u[i])
                temp.append((1 - a_ir) * P[i - dis - 1] + a_ir * P[i - dis])
            P[k - self.k + r - dis:k - sk + 1 - dis] = np.array(temp)
    ...
    def get_knots(self, method="average"):
        ...
        knots = np.zeros(self.k + 1).tolist()
        ...
        num = self.m - self.k  # select n+1 paras
        ...
        for j in range(1, self.n - self.k + 1):
            ...
            for i in range(j, j + self.k - 1 + 1):
                k_temp += paras_knots[i]
            k_temp /= self.k
            knots.append(k_temp)

        add = np.ones(self.k + 1).tolist()
        knots = knots + add
...
def path3D_to_bs(saved=False):
    ...
    for scene_label in scene_list:
        ...
        for label, path_ in zip(path_dict.keys(), path_dict.values()):
            ...
            bs = BS_curve(cp_num, 3, min(cp_num, 10))
...
def route3D_to_bs(saved=False):
    # 读取线缆路径散点
    pathdir = '../../data/LAB_imgs_1028_DLO/route3D_select/'
    bsdir = '../../data/LAB_imgs_1028_DLO/route3D_select_bs/'
    visdir = '../../data/LAB_imgs_1028_DLO/route3D_select_bs_vis/'
