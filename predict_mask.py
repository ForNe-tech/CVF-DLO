...
os.makedirs("EWD/test_masks_r50", exist_ok=True)

for img_name in tqdm(img_list):

    source_img = cv2.imread(os.path.join("EWD/test_imgs", img_name), cv2.IMREAD_UNCHANGED)
    mask_img = network_seg.predict_img(source_img)

    cv2.imwrite(os.path.join("EWD/test_masks_r50", img_name), mask_img)